...
$(function() {
    ...
    function setupApp() {
        ...
        $.getJSON("../config/inputStrings_" + language + ".json", function(data){
            ...
            socket.on('side', function(data){
                teamSide = data;
                $("#text").append("<h1>"+GAMEVOTING[teamSide + "Name"]+"</h1>");
            });
    ...
    function animationShowInstructionGame(){
        setTimeout(function () {
            $("#info").append("<h2>"+GAMEVOTING[teamSide + "InitMsg"]+"</h2>").addClass('up');
            $("#arrow").append("<h3> > </h3>").addClass('down');
        }, 1000);
    }
